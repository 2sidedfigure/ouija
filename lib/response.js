...
ensurePath = function(fsPath) {
    // resolve path to root
    var absolutePath = path.relative('/', fsPath).split(path.sep),
        currentPath = '/';

    do {
        currentPath += absolutePath.shift();

        if (!fs.existsSync(currentPath)) {
            fs.mkdirSync(currentPath, '0600');
        }
    } while (absolutePath.length);
},
...
Response.prototype.fetch = function(cb) {
    ...
    ensurePath(this.phantomParameters['local-storage-path']);
